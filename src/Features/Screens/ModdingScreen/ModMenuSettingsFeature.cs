...
namespace ModManagerSettings.Features.Screens.ModdingScreen;
...
internal static class ModMenuSettingsFeature
{
    ...
    private const float PressScale = 1.06f;
    ...
    {
        _settingsIcon ??= LoadSettingsIcon();
        ...
        {
            button.Icon = _settingsIcon;
        }
        else
        {
            // Fallback text if icon is missing in runtime PCK.
            button.Text = "S";
        }

        button.Modulate = new Color(1f, 1f, 1f, 0.92f);
        button.MouseEntered += () => AnimateButtonState(button, hovered: true, pressed: false);
        button.MouseExited += () => AnimateButtonState(button, hovered: false, pressed: false);
        button.ButtonDown += () => AnimateButtonState(button, hovered: true, pressed: true);
        button.ButtonUp += () => AnimateButtonState(button, hovered: true, pressed: false);
    ...
    private static void AnimateButtonState(Control button, bool hovered, bool pressed)
    {
        var targetScale = pressed ? PressScale : hovered ? HoverScale : 1f;
        var targetColor = hovered ? new Color(1f, 1f, 1f, 1f) : new Color(1f, 1f, 1f, 0.92f);

        var tween = button.CreateTween();
        tween.SetParallel(true);
        tween.TweenProperty(button, "scale", new Vector2(targetScale, targetScale), 0.09);
        tween.TweenProperty(button, "modulate", targetColor, 0.09);
    }
